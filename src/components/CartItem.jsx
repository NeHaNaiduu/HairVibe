...
const CartItem = ({name,image,price,quantity, totalPrice,id}) => {
    ...
    const decrement=()=>{
        dispatch(removeFromCart({id,quantity,price, totalPrice}))
    }

  return (  
    <div className='container'>
        <div className='border flex justify-between py-5 px-10 items-center'>
            <img src={image} alt="" className='w-32'/>
            <div className='flex flex-col items-start w-1/3'>
                <h3 className='font-semibold mb-2'>{name}</h3>
                <h3>{price}</h3>
                <h3 className="mb-4">{totalPrice}</h3>
